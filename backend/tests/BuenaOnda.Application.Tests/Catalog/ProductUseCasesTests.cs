...
public class ProductUseCasesTests
{
    ...
    private static readonly OptionInput Option = new(1000m, true, null, null);
    ...
    {
        var view = await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, Option);
    ...
    {
        await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, Option);

        await Assert.ThrowsAsync<ConflictException>(() => Create.ExecuteAsync(" AGUA ", null, null, _bebidas.Id, Option));
        await Create.ExecuteAsync("Agua", null, null, _postres.Id, Option);
    }

    [Fact]
    public async Task Create_requires_existing_active_category_and_option()
    {
        await Assert.ThrowsAsync<ValidationException>(() => Create.ExecuteAsync("X", null, null, null, Option));
        await Assert.ThrowsAsync<NotFoundException>(() => Create.ExecuteAsync("X", null, null, Guid.NewGuid(), Option));
        await Assert.ThrowsAsync<ValidationException>(() => Create.ExecuteAsync("X", null, null, _bebidas.Id, null));
        ...
        await Assert.ThrowsAsync<ConflictException>(() => Create.ExecuteAsync("X", null, null, inactive.Id, Option));
    }

    [Fact]
    public async Task Update_moving_to_category_with_same_name_is_conflict()
    {
        var agua = await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, Option);
        await Create.ExecuteAsync("Agua", null, null, _postres.Id, Option);
    ...
    {
        var agua = await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, Option);
        await Create.ExecuteAsync("Flan", null, null, _postres.Id, Option);
