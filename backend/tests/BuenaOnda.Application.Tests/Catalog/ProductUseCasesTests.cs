...
public class ProductUseCasesTests
{
    ...
    [Fact]
    public async Task Create_saves_product_with_price_and_derived_indicators()
    {
        var view = await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, 1000m, true);

        Assert.Equal(1000m, view.Price);
    ...
    {
        await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, 1000m, true);

        await Assert.ThrowsAsync<ConflictException>(() => Create.ExecuteAsync(" AGUA ", null, null, _bebidas.Id, 1000m, true));
        await Create.ExecuteAsync("Agua", null, null, _postres.Id, 1000m, true);
    }

    [Fact]
    public async Task Create_requires_existing_active_category_and_option()
    {
        await Assert.ThrowsAsync<ValidationException>(() => Create.ExecuteAsync("X", null, null, null, 1000m, true));
        await Assert.ThrowsAsync<NotFoundException>(() => Create.ExecuteAsync("X", null, null, Guid.NewGuid(), 1000m, true));
        await Assert.ThrowsAsync<ValidationException>(() => Create.ExecuteAsync("X", null, null, _bebidas.Id, null, true));
        ...
        await Assert.ThrowsAsync<ConflictException>(() => Create.ExecuteAsync("X", null, null, inactive.Id, 1000m, true));
    }

    [Fact]
    public async Task Update_moving_to_category_with_same_name_is_conflict()
    {
        var agua = await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, 1000m, true);
        await Create.ExecuteAsync("Agua", null, null, _postres.Id, 1000m, true);

        await Assert.ThrowsAsync<ConflictException>(() =>
            new UpdateProduct(_repos, _repos, _repos).ExecuteAsync(agua.Id, "Agua", null, null, _postres.Id, 1000m));
    }
    ...
    public async Task Update_changes_information_and_allows_keeping_own_name()
    {
        ...
        var updated = await new UpdateProduct(_repos, _repos, _repos)
            .ExecuteAsync(agua.Id, "AGUA", "Fría", "https://cdn.example.com/a.png", null, 1200m);
    ...
    {
        var agua = await Create.ExecuteAsync("Agua", null, null, _bebidas.Id, 1000m, true);
        await Create.ExecuteAsync("Flan", null, null, _postres.Id, 1000m, true);
