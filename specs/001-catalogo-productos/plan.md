...
| IV. Stack tecnológico | Sí | Cumple con dependencia | Stack obligatorio intacto. Complementarias declaradas y justificadas (Npgsql, xUnit, ejecutor de pruebas de Angular). PostgreSQL 17 usado mediante `psql-17`. La autenticación JWT/Identity de las funcionalidades administrativas la entrega la feature de usuarios y roles (ver Complexity Tracking). |
...
|--------------------|-------------------|-----------------------------------|
| Los endpoints administrativos quedan sin autenticación hasta que exista la feature de usuarios y roles (Principio IV exige Identity y JWT en funcionalidades administrativas) | Los requerimientos y la spec de esta feature excluyen autenticación, roles y permisos; implementarla aquí decidiría roles y permisos por suposición | Implementar una autenticación provisional: se descarta por introducir decisiones reservadas a la especificación de usuarios y roles. Esta feature no es desplegable a un entorno accesible hasta que se aplique esa protección; los endpoints se agrupan bajo un prefijo administrativo único para que se aplique de una sola vez. |
